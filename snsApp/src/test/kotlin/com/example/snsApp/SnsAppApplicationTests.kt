...
//	postTable
	...
	fun `postテーブルへのGETリクエストはOKステータスを返す`() {
		val response = restTemplate.getForEntity("http://localhost:$port/postTable", String::class.java)
		assertThat(response.statusCode, equalTo(HttpStatus.OK))
	}
	@Test
	fun `postテーブルへのGETリクエストはpostsオブジェクトのリストを返す` () {
		val response = restTemplate.getForEntity("http://localhost:$port/postTable", Array<GetfromPostTable>::class.java)
...
//		val timestamp = Timestamp(System.currentTimeMillis())
		...
		val response = restTemplate.postForEntity("http://localhost:$port/postTable", request,String::class.java)
		assertThat(response.statusCode, equalTo(HttpStatus.OK))
	}
	@Test
	fun `Postリクエストはpostオブジェクトを格納する` () {
		val response1 = restTemplate.getForEntity("http://localhost:$port/postTable", Array<GetfromPostTable>::class.java)
		val post1 = response1.body!!
//		投稿を行う
		val request = PostRequest(1,  Timestamp(System.currentTimeMillis()),"やあ", 2)
		restTemplate.postForEntity("http://localhost:$port/postTable",request,String::class.java)

		val response2 = restTemplate.getForEntity("http://localhost:$port/postTable", Array<GetfromPostTable>::class.java)
...
//commentへのテストはOKだけ。
	...
	fun  `Postリクエスト(comment)はOKステータスをだす` () {
		...
		var response = restTemplate.postForEntity("http://localhost:$port/comments", request, String::class.java)
		assertThat(response.statusCode, equalTo(HttpStatus.OK))
	}
...
//	delete
	...
	fun `DeleteリクエストはOKステータスを返す` () {
		val requestEntity = RequestEntity.delete("http://localhost:$port/postTable/5",String::class.java).build()
...
//		取得する
		...
		val post1 = response1.body!!
//		削除する
		restTemplate.delete("http://localhost:$port/postTable/{id}", post1[post1.size - 1].id)
//		再び取得する
		val response2 = restTemplate.getForEntity("http://localhost:$port/postTable", Array<GetfromPostTable>::class.java)
		val post2 = response2.body!!
//		削除後は配列の長さが1短くなる
	...
	fun `PutリクエストはOKステータスを返す` () {
		val requestEntity = RequestEntity.put("http://localhost:$port/posts/3/good",String::class.java)
